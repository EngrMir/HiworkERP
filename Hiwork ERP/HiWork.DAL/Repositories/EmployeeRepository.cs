...
namespace HiWork.DAL.Repositories
{
    public interface IEmployeeRepository : IBaseRepository<Employee> 
    {
        ...
        bool CheckEmployeeByEmployeeID(Employee model);
        bool DeleteEmployee(Guid Id);
    }
    public class EmployeeRepository : BaseRepository<Employee, CentralDBEntities>, IEmployeeRepository, IDisposable
    {
        ...
        public bool CheckEmployeeByEmployeeID(Employee model)
        {
            bool status = false;
            try
            {
                var employee = _dbContext.Employees.Where(e => e.EmployeeID == model.EmployeeID && e.ApplicationID == model.ApplicationID).SingleOrDefault();
                if (employee != null)
                    status = true;
            }
            catch(Exception ex)
