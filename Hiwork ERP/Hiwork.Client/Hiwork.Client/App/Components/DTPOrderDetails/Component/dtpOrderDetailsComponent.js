...
angular.module("hiworkApp").component('dtpdetails', {
    templateUrl: 'app/Components/DTPOrderDetails/Template/dtpOrderDetails.html',
    controllerAs: "vm",
    bindings: {
        routes: "=",
        services: "=",
    ...
    controller: dtpOrderDetailsController
})

function dtpOrderDetailsController($scope, $compile, $uibModal, $filter, estimationService, AppStorage, appSettings, sessionFactory, ajaxService, $http, $stateParams, $parse, $localStorage, uiCalendarConfig, $sce, $rootScope, $state, $templateCache) {
    var vm = this;
    vm.rowCollection = [];
    vm.EstimationModel = {};
    vm.EstimationAction = {};
    ...
    vm.$onInit = function () {
        if ($stateParams.id != "" && $stateParams.id != null && $localStorage.DtpEstimation != null && $stateParams.id === $localStorage.DtpEstimation.ID) {
            vm.EstimationModel = $localStorage.DtpEstimation;            
    ...
    vm.goBack = function (id) {
        if (id == null) {
            var dtpID = $localStorage.dtpID;
            $state.go('DTPEstimation', {
                id: dtpID
            });
        }
        else {
