...
angular.module("hiworkApp").component('transcriptionOrder', {
    templateUrl: 'app/Components/TranscriptionOrderDetails/Template/transcriptionOrderDetails.html',
    controllerAs: "vm",
    bindings: {
        routes: "=",
        services: "=",
    ...
    controller: transcriptionOrderDetailsController
})

function transcriptionOrderDetailsController($scope, $compile, $uibModal, $filter, estimationService, AppStorage, appSettings, sessionFactory, ajaxService, $http, $stateParams, $parse, $localStorage, uiCalendarConfig, $sce, $rootScope, $state, $templateCache) {
    var vm = this;
    vm.rowCollection = [];
    vm.EstimationModel = {};
    vm.EstimationAction = {};
    ...
    vm.$onInit = function () {
        if ($stateParams.id != "" && $stateParams.id != null && $localStorage.TranscriptionEstimationModel != null && $stateParams.id === $localStorage.TranscriptionEstimationModel.ID) {
            vm.EstimationModel = $localStorage.TranscriptionEstimationModel;
    ...
    $scope.$on("selectedStaff", function (event, response) {
        ...
        var obj = {
            ...
            LengthMinute: response[0].Item.LengthMinute,
    ...
    vm.goBack = function (id) {
        ...
        $state.go('TranscriptionEstimation', {
            id: id
        });
    };
    ...
    vm.calculateHands = function () {
        if (vm.Hands.ID > 0) {
            ...
            vm.OrderModel.OutwardsSalesPersonShare = ((r.OutwardsSalesPersonShare / 100) * vm.OrderModel.Profit);
